...
public class Player {
	...
	public Player (SpectrumVisualizer spectrumVisualizer, String audioFile) {
	...
	public void start(){
		...
		try {
			...
			File audioFile = new File("AudioFiles\\"+this.audioFile+".wav");
			...
			while ((bread = in.read(bytes)) != -1 && View.running){
			...
			spectrumVisualizer.drawWaveForm2(samples);

			out.write(bytes, 0, bread);
			}
			spectrumVisualizer.finished();
